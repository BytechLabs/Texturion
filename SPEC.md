...
| `GET /v1/calls` | M | #129 call log from the `calls` read model: `{ id, caller_e164, contact_name, phone_number_id, conversation_id, outcome ('answered'\|'voicemail'\|'missed'\|null), forward_seconds, started_at }`; `?outcome=` filter; cursor on `(started_at, id)`; #106 number-access deny list applied in-SQL |
