...
- **Metered:** outbound SMS segments, and calling minutes — INCLUDED on every plan since D42/#134 (D36 mechanics: billed-leg minutes both directions; 2,500 included on Starter / 6,000 on Pro, then 1¢/min under the same overage cap). Outbound **MMS meters as 3 segments**. Notes are free. Inbound SMS and MMS are free and unmetered (market table-stakes; COGS ~0.7¢/segment absorbed).
...
- **Supabase (Pro plan, $25/mo, single US region `us-east-1`):**
  ...
  - **Realtime** — **Broadcast-from-Database** (never `postgres_changes`), private topics authorized by RLS on `realtime.messages`. An event belonging to a number goes to `company:{id}:number:{n}`, admitted only to members whose effective access to that number is not `none`; company-wide events go to `company:{id}` — see §8, D85 and D88.
...
S1. Ensure messaging profile:
    POST /v2/messaging_profiles { name: company_id, webhook_url:
      https://api.loonext.com/webhooks/telnyx, webhook_failover_url:
      https://api.loonext.com/webhooks/telnyx,   ← same route; enables Telnyx's
...
|---|---|
| Create brand (standard & sole-prop) | `POST /v2/10dlc/brand` (payload includes `webhookURL` + `webhookFailoverURL` = `https://api.loonext.com/webhooks/telnyx`) |
...
**Removed (owner direction, 2026-07).** The original D4 behavior — the first outbound-first message ever sent to a contact auto-appended `— {Business name}. Reply STOP to opt out` (CASL identification + CTIA), tracked via `contacts.first_identification_sent_at`, with a composer preview of the footer — no longer exists. No message is decorated; messages send exactly as typed. **The compliance trade-off was accepted knowingly:** first messages are no longer guaranteed to carry identification/opt-out text, which can weaken 10DLC standing and invite carrier filtering. Inbound STOP honoring and the `recipient_opted_out` send block (above) are unchanged. `contacts.first_identification_sent_at` remains in the schema but is never written (§6).
...
- **CORS:** `apps/api` serves CORS for the exact web origin **`https://app.loonext.com` only** — allow methods `GET, POST, PATCH, PUT, DELETE`; allow headers `Authorization, X-Company-Id, Idempotency-Key, Content-Type`. No wildcard origins. `/webhooks/*` routes send **no** CORS headers.
...
| `POST /v1/calls` | M | D38 click-to-call bridge: `{ conversation_id }` → dials the member's own cell from the business number (AMD-gated), webhook bridges to the customer; 202 `{ status: 'dialing', call_session_id }`. Gates: #106 'text' level, live subscription (D42: no module — calling is included), `call_cell_e164` set AND VERIFIED (D40), no in-flight outbound session for the conversation (#133), D36 voice cap (`usage_cap_reached`). Dial-then-persist failures compensate with an agent-leg hangup |
| `GET`/`PUT /v1/calls/cell` | M | D38/D40: the member's OWN bridge cell (`company_members.call_cell_e164`, NANP; self-service per membership) → `{ call_cell_e164, verified, code_sent? }`. PUT of a new/unverified cell texts a 6-digit code from the business number (raw Telnyx send — no messages row, never metered; 60s cooldown, 6/24h window, VERIFY_RATE_LIMITER on the target cell, opt-out pre-checked) |
| `POST /v1/calls/cell/verify` | M | D40: `{ code }` → attempt-capped (5) hash check against the pending code (10 min TTL); success stamps `call_cell_verified_at`, unblocking the bridge |
| `GET /v1/calls` | M | #129 call log from the `calls` read model: `{ id, caller_e164, contact_name, phone_number_id, conversation_id, outcome ('answered'\|'voicemail'\|'missed'\|null), direction ('inbound'\|'outbound'), forward_seconds, started_at }`; `?outcome=` filter; cursor on `(started_at, id)`; #106 number-access deny list applied in-SQL |
| `POST /v1/conversations` | M | Start outbound-first: `{ contact_id \| phone_e164, phone_number_id, body, consent_attested: true, quiet_hours_confirmed? }` + `Idempotency-Key` → creates/attests contact, conversation (status `open`), sends first message (all send gates apply). If an open conversation already exists for the (contact, number) pair (`conversations_open_uq`), the message is appended to it instead — gates and consent attestation still apply and are recorded — and that conversation is returned (`200`) |
...
> while reading was checked per number, so a member denied a number still received
> every id-only event for conversations on it — existence, direction and timing, in
> real time. No content leaked, but volume and rhythm did. That was D85's recorded
> exposure, and it is **closed**, not still accepted.
>
> - **An event that belongs to a number** publishes to
>   `company:{company_id}:number:{phone_number_id}` and nowhere else. Joining that
>   topic is authorized in SQL by the `realtime.messages` policy, which admits a
>   member only when `member_number_level` is not `none` — the same effective-access
>   rule the API uses, called from one place rather than reimplemented (D88).
> - **An event that belongs to the whole workspace** publishes to
>   `company:{company_id}`: `registration.updated` (a 10DLC registration authorizes
>   every number), `read.notifications` (one watermark per person),
>   `access.changed` and `number_set.changed` (the two "ask again" signals, which
>   must reach a member precisely when their number set is about to change).
>
> **One deliberate exception, and it is not a fallback for the transition:**
> `call.updated` for a call whose number was deleted has no number to be scoped to
> (`calls.phone_number_id` is `on delete set null`) and no surviving restriction to
...
- **Prices:**
  ...
  - Voice overage (D36, on the `voice_seconds` meter, hung off the Calling module product): `loonext_starter_voice_overage` graduated tier 1 `up_to=150000` (2,500 min × 60) at $0, tier 2 `up_to=inf, unit_amount_decimal='0.016666666667'` (1¢/60 per second); `loonext_pro_voice_overage` tier 1 `up_to=360000` then the same per-second rate. D42/#134: attached to EVERY subscription (checkout, plan-change swap, mirror-pass convergence) — calling is included on every plan; the retired $8 licensed item strips with prorated credit via the daily retired-price sweep.
...
- **Voice pipeline (D36) mirrors the above:** on every forward-leg `call.hangup`, `call_records` records the leg (unique `call_leg_id`; a rang-out leg records ZERO billable seconds) and its RAW `billable_seconds` are reported to the `voice_seconds` meter (identifier = `call_leg_id`; the metered price rates 1¢ per 60 s), with `call_records.stripe_reported_at` as the local exactly-once stamp (hourly `reportUnreportedVoiceUsage` re-reports NULL rows and sweep-stamps non-billable strays; non-billable rows are stamped at insert). Allowance/alerts/cap read the SAME `api_period_forward_seconds` sum — which since D38 covers BOTH far-party legs (`forward` inbound + `out_customer` outbound: one calling-minutes pool) — so the bill can never diverge from the displayed figures; forwarding pauses (USER_BUSY reject → missed-call text) and outbound dialing refuses (`usage_cap_reached`) at allowance × `overage_cap_multiplier` (D42: the grandfathered 300-minute legacy line retired with the module — plan allowances for everyone). Every subscription mirror pass converges the voice metered item onto EVERY live subscription (attach/swap/dedupe) and voice-binds the workspace's numbers.
...
- **Front door:** `POST /v1/billing/checkout` is limited to **10 req/min/IP** by the `CHECKOUT_RATE_LIMITER` binding, evaluated first in the handler — before the body is parsed and before any read, since the cost being bounded is the whole request. This used to read "Cloudflare WAF rate-limiting rule"; on 2026-08-09 that rule was measured absent (40 back-to-back POSTs from one IP, no 429 at any point), so the limit now lives in the repo where it can be read from a clone, reviewed in a diff, and tested. **A control that exists only in a dashboard cannot be verified from here at all** — a documented protection and a missing one look identical — so prefer the binding for anything this spec claims. Supabase Auth traffic goes browser → `<project>.supabase.co` directly (§3) — it is **not** behind Loonext's Cloudflare zone, so it is protected by Supabase Auth's built-in rate limits plus **Cloudflare Turnstile enabled through Supabase Auth's captcha-protection setting** (Auth → Attack Protection → CAPTCHA, provider "Turnstile") — that setting *is* the signup Turnstile mechanism. Read back from the project's live auth config on 2026-08-09: enabled, provider `turnstile`. That is a hosted setting with no repo artifact, so it is true as of that date and nothing here would notice if somebody turned it off — re-read it, don't assume it.
- **Browser Integrity Check is on for the zone**, and it refuses one client signature before the Worker runs: a `User-Agent` beginning `Python-urllib` gets Cloudflare's error 1010 (`HTTP 403`, `text/plain`, 17 bytes) instead of our answer. Case-sensitive and prefix-anchored — `python-urllib`, `Python-requests`, `okhttp`, `CFNetwork`, `curl` and an absent user agent are all served, so **no client of ours is affected** and there is nothing here to pin (#596, re-measured 2026-08-10). It is kept because it costs us nothing and the signature it blocks is overwhelmingly scanners. What it does cost is measurement: Python's `urllib` is what a probe script reaches for by default, and the block page is a *confident wrong answer* carrying its own `Cache-Control` and `X-Frame-Options`. It produced one during the #586 audit. **Any script probing this API must refuse to grade a response it cannot identify as ours** — `scripts/ops/verify-api-headers.mjs` is the pattern.
...
- **Secret scanning:** GitHub secret scanning and **push protection** are both **on** (verified through the API, #601), so a commit carrying a recognised credential is refused *before it reaches the remote*. That is what "we scan for secrets" means here now: previously it meant only the `Security` workflow's gitleaks pass, which reads the whole history after the fact — a real hit there costs a history rewrite and a rotation rather than a rejected push. Both are kept: push protection uses GitHub's provider list, which is wider than `.gitleaks.toml` and updated without us, and gitleaks covers the shapes specific to this stack. **Dependabot security updates** are on. **The dependency graph is off** (an organisation setting; see `docs/RELEASING.md`), which is why `Dependency review` cannot evaluate a pull request.
- **PII / telemetry policy:** message bodies, names, addresses, and phone numbers never reach Sentry or PostHog. **True as of 2026-08-09, and not before** (#585): outbound-fetch breadcrumbs carried the full URL — query string and all — from 2026-07-01 until the cut shipped, so typed search terms, geocoded addresses and email addresses did reach Sentry for those five weeks. Nothing is being added; the existing events age out by 2026-11-07 at the outside. Recorded as **R10** in `docs/ACCEPTED-RISKS.md`, with the one open question — who can read that Sentry organisation — as its trigger. Console breadcrumbs are dropped outright rather than pattern-matched, because a commitment this sentence makes cannot rest on a redaction list that is definitionally incomplete. Sentry `beforeSend` strips request/response bodies and redacts E.164 patterns; `sendDefaultPii` off. PostHog: UUIDs, counts, and feature events only; autocapture masked; no session replay on inbox/conversation pages. Logs reference IDs, never bodies or destination numbers. `messaging_registrations.data` stores the **full EIN/BN** (business identifier — required for brand submission, which happens later at webhook time) but **SSN/SIN as last-4 only**: full SSN/SIN are never collected or persisted (Telnyx's Sole Proprietor path requires only the last-4, §4.4).
...
| Stale-calls sweeper | `0 * * * *` (hourly) | #133: `api_sweep_stale_calls` flips call sessions in-flight (outcome NULL) >4h to 'missed' — /calls stays honest, the per-conversation dial guard re-opens | State-selected (outcome NULL + age); re-runs are no-ops |
...
13. **Compliance UX.** Consent attestation on outbound-first compose (`POST /v1/conversations`), quiet-hours confirm dialog + event, first-message footer + preview *(footer since removed — D4 reversal, 2026-07)*, manual opt-out/revoke actions + banners, opted-out composer lockout. ✅ Outbound-first without attestation is impossible; footer appeared exactly once per contact while it existed.
