...
| `POST /v1/calls` | M | D38 click-to-call bridge: `{ conversation_id }` → dials the member's own cell from the business number (AMD-gated), webhook bridges to the customer; 202 `{ status: 'dialing', call_session_id }`. Gates: #106 'text' level, live subscription, voice module, `call_cell_e164` set, D36 voice cap (`usage_cap_reached`) |
| `GET`/`PUT /v1/calls/cell` | M | D38: the member's OWN bridge cell (`company_members.call_cell_e164`, NANP; self-service per membership) |
