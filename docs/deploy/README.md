...
| 5 | Set all 21 API Worker secrets + GitHub Actions secrets, deploy both Workers, custom domains, register live webhook URLs | [05-workers-deploy.md](./05-workers-deploy.md) | Live `api.` + `app.` Workers |
...
- **Node ≥ 22**, **pnpm** (CI pins Node 22 — `.github/workflows/ci.yml:55`).
...
- **`NEXT_PUBLIC_API_URL` must exist as a GitHub Actions secret** before the first
  ...
  without it. (CI itself uses a fixed placeholder — `.github/workflows/ci.yml:44-47`
