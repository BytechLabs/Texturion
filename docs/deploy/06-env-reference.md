...
| `STRIPE_PAUSE_PRICE_ID` | no | `stripe:setup` output. #277 seasonal pause — the recurring licensed price that replaces the plan price while a workspace is paused (number, history and 10DLC registration all held; sending off). **Unset = no pause offer anywhere**, and never a free one. Set the AMOUNT in Stripe; nothing in the repo hardcodes it. **Never rotate or unset this while any workspace is paused** — the pause is identified by this exact price id on the subscription item, so changing it strands everyone currently paused: `POST /v1/billing/resume` refuses (it cannot tell which item to swap back), and there is no in-app way out for them. Reprice by editing the price in Stripe if you can; if you must issue a new price id, resume every paused workspace first. The failure is on OUR side of the ledger — a stranded workspace stays blocked from sending, never given service for free — but it is a support conversation per customer | `price_xxxxxxxxxxxx` |
