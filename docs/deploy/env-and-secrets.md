...
| `STRIPE_MODULE_VOICE_PRICE_ID` | Calling (voice) add-on licensed price ($8/mo). Same schema-optional / launch-required semantics as above. | Printed by `stripe:setup` — `scripts/stripe-setup.ts:208-210`. | Secret. `env.ts:65` |
| `OPS_ALERT_EMAIL` | #121 storage-abuse alerts: ops recipient for the "[ops] storage abuse tier" emails. Optional — defaults to support@loonext.com (routes to the founder via Email Routing). | Plain email address. | Var (not secret). |
| `STRIPE_MODULE_EXTRA_STORAGE_PRICE_ID` | RETIRED (#121) — the add-on no longer sells, but KEEP this set: the daily reconcile identifies and strips the price from existing subscriptions (prorated credit). Unsetting it silently strands subscribers on a dead $5 line item. | Printed by `stripe:setup` — `scripts/stripe-setup.ts:208-210`. | Secret. `env.ts:66` |
...
| `RESEND_FROM` | The `from` header, e.g. `Loonext <notifications@loonext.com>` (`apps/api/src/email/resend.ts:35`; fixture `test/support.ts:26`; note `env.ts:42`). | Operator-chosen; the domain must be a **verified sending domain** in Resend. | Secret (config value). `env.ts:43` |
...
| `NEXT_PUBLIC_API_URL` | Base URL of the api Worker the browser calls, e.g. `https://api.loonext.com` (validated as a URL, `apps/web/src/env.ts:6`). Should equal the api's `API_ORIGIN`. | Operator-chosen; the api Worker's public URL. | **Public.** Build-time inlined. `env.ts:6` |
...
| `NEXT_PUBLIC_APP_ORIGIN` | **OPTIONAL.** The app portal origin (production: `https://app.loonext.com`; must equal the api's `APP_ORIGIN`). Activates the D27 marketing/app host split: the middleware's first gate serves **only** marketing pages on `loonext.com` (app-surface paths 308 to the app origin; `www` canonicalizes to the apex) and **only** the product on this origin (`apps/web/src/env.ts:11-16,30`, `apps/web/src/lib/hosts.ts`, `docs/DECISIONS.md` D27). Blank/unset (dev, CI, previews) = no gating. Requires `loonext.com`, `www.loonext.com`, and `app.loonext.com` all attached to the one web Worker. Supabase/Stripe return URLs stay on `APP_ORIGIN` unchanged. | Operator-chosen; the app hostname on the web Worker. | **Public.** Build-time inlined. `env.ts:16` |
...
| `NEXT_PUBLIC_APP_ORIGIN` **(optional)** | `deploy.yml:27-30` | App origin for the D27 host split (§2). Blank/unset = no host split; production value `https://app.loonext.com`. |
