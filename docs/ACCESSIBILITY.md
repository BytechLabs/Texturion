...
|---|---|---|
| **2.1.1** Keyboard | A complete path — rail → list → thread → composer → panel — with focus never trapped in a scroll region, and `Tab` order following visual order across panes (§7) | Focus order is a property of a rendered, focused page. A source scan cannot see it. |
| **2.4.7 / 2.4.11** Focus Visible / Appearance | Visible focus throughout (§7); 2.2 adds a measurable contrast-and-area bar | Needs a rendered focus ring compared against its surround. `theme-audit.mjs` names this as deliberately outside its current pass rather than shipping a noisy near-miss gate. |
