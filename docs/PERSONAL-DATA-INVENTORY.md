...
| `task_calendar_links` | the task title and exact agreed/provider scheduling fields for one mapped calendar occurrence; description equality is a **SHA-256 hash**, never another plaintext copy (#245/D137) | Life of the mapping; purged with the workspace. Attention snapshots survive only so a person can resolve a conflict without either edit being destroyed |
| `calendar_outbox` | the same task scheduling snapshot while a provider write is pending, plus lease/error metadata. No plaintext description (#245/D137) | Completed attempts remain as operational evidence until workspace teardown; live attempts are cancelled on disconnect/replacement |
| `calendar_reminder_replans` | task and requesting-member references plus retry/lease/error metadata for route-driven reminder replanning; no reminder body or calendar snapshot (#245/D137) | Completed attempts remain as operational evidence until workspace teardown; live attempts are cancelled when the task is cancelled or during workspace teardown |
...
| `calendar_feed_tokens` | a **hash** of the subscription credential — never the token — plus who it belongs to and when it was last polled (#245) | Lives until the member rotates or revokes it, which is the point: a calendar subscription that expired on a schedule would silently stop updating somebody's week. Rotation revokes the old row rather than editing it, so the audit of what was live when survives. Goes with the workspace in the teardown |
| `calendar_connections` | which member connected which provider account/calendar, provider cursor/liveness, the refresh credential as AES-GCM **ciphertext + IV + key version** — never plaintext — bounded content-free timestamps/count/window for true conflicts in the preceding 7 days, and content-free one-time owner-disclosure lease/retry metadata (#245/D137) | One live connection per member/workspace. In-place reauthorization retains the row; final revocation wipes encrypted credential fields immediately. A cleanup-failure tombstone remains long enough to deliver its owner disclosure; all tombstone/sync/rate evidence goes with workspace teardown |
| `oauth_states` | member/provider, a one-use state **hash**, and the PKCE verifier as AES-GCM ciphertext (#245) | Minutes: consumed once or expired. Also purged with the workspace so an abandoned flow cannot outlive it |
...
| `quotes` | an amount, a currency, an expiry, and **what the work is in the crew's own words**, joined to the conversation and contact it was quoted to (#287). The description is free text a person typed and can name somebody or somewhere, so it is listed here rather than under "no personal data". No line items, no tax, no card or bank details: the money is handled by `payment_requests` | Life of the workspace; erased with it, and named in `purge_workspace_step` |
...
| `webhook_subscriptions` | opaque Google/Graph subscription and resource ids plus a client-state **hash**; no event body or customer content (#245) | Renewed for the live connection, revoked on disconnect/replacement, and purged with the workspace |
| `api_keys` | a **name** the owner typed, a **token hash**, the **user id** who created it and the one who revoked it. No contact data, and the token itself is never stored (#243) | Life of the key. A revocation is a STAMP, not a delete: "when was that turned off, and by whom" is the first question after an incident and a deleted row cannot answer it. Rows go with the workspace in the teardown — a workspace that has been erased must not leave a live token that still resolves to its id |
