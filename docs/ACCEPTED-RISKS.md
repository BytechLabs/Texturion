...
| **Bound** | It ages out on its own, and the ceiling is published: Sentry retains error events for **30 days on Developer and up to 90 on Team and Business** (sentry.io/pricing, read 2026-08-09). So the last of the affected window is gone by **2026-09-08** at 30 days, or **2026-11-07** at 90 — no action, no dependency. The recording links inside it are already dead whatever the plan: Telnyx presigns them `X-Amz-Expires=600`, ten minutes, so every one logged expired the same hour. What remains has to be *read by somebody*, which makes the reader population the whole of the severity. |
| **Why accepted** | Purging is possible but blunt — Sentry's API deletes issues, not fields, so it means destroying the error history for the window as well, on a product whose observability is one Worker's Sentry project. If the reader population is one person who is also the data controller, that trade is plainly not worth making for data that expires by itself. **That premise is the entry's load-bearing assumption and it has not been verified** — see the trigger. |
| **Trigger to revisit** | **Immediately, on the one open fact:** whether anyone besides the founder can read the Sentry organisation `hayaturehman-ahmadzai`. If the member list is one person, this entry stands as written and needs nothing. **If anyone else has access, purge the window rather than waiting it out** — a personal-scale trade stops applying the moment the audience is not the controller. The check is Sentry → Settings → Members; the CI token this repo holds is scoped `org:ci` and cannot read it, which is why it is a trigger and not an answer. Also fires if a DPA or a customer questionnaire asks when the §10 commitment became true, since the honest answer is this entry. |
| **Not a trigger** | Time passing on its own, or the entry looking untidy beside the fix. **Delete this entry once the window has aged out** (2026-11-07 at the outside) — a resolved risk is pruned, not archived, and this one has an expiry date rather than a review date. |
