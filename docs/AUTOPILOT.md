...
- Stage explicit paths only, never git add -A: the repo is public and scratch artifacts, decompiled vendor code, or secrets must never enter history. Commits are conventional and drive release-please: feat/fix/perf subjects are published VERBATIM in user-facing release notes, so write each as one plain-English customer-visible outcome in present tense; internal work uses chore/refactor/docs/test/ci, which stay out of the notes. Never bundle two outcomes in one subject.
- Code health is a standing track, not an afterthought: on a regular cadence run a cleanup pass over recently-touched areas - dedupe near-identical code into shared helpers, delete dead code and unused assets, collapse redundant abstractions, align naming and idioms - shipped as refactor/chore commits behind the full gates. Leave every file you touch simpler than you found it; never leave TODOs, stubs, or commented-out code behind.
...
- In the rare case a founder message arrives mid-loop, it is P0 input: file each report as an issue with your analysis, fix in priority order, and answer any question directly. Otherwise assume total silence for weeks and act accordingly.
