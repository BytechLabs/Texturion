...
final class HandoverFunnelRunTests: XCTestCase {
    ...
    private final class Recorder: HTTPClient, @unchecked Sendable {
        ...
        /// `withLock` at both sites: `lock()`/`unlock()` are `@available(*, noasync)`,
        /// because holding a lock across a suspension point is a deadlock waiting to
        /// happen, and `data(for:)` is async. CI said so in as many words.
    ...
    private func makeScope(ours: Recorder) -> SettingsScope {
        // ONE store, in memory, handed to both the graph and the client.
        //
        // The memory part is #599: the simulator host has no keychain, and the
        // keychain-backed store silently keeps nothing there — which is how the first
        // version of this file failed three times reporting the person was signed out.
        // The ONE part is the Android twin's lesson: the funnel reads the token through
        // the repository and SAVES the refreshed one through `graph.sessionStore`, so
        // two stores read as signed out and it never reaches the code under test.
        let store = SessionStore(storage: InMemorySessionStorage())
        let graph = AppGraph(sessionStore: store)
        store.save(
            Session(
                accessToken: "token-1",
