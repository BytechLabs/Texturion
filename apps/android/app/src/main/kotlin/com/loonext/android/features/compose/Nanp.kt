...
/**
 * NANP destination helpers — a Kotlin port of the essentials of
 * packages/shared/src/nanp.ts (compiled from NANPA's NPA report, file date
 * 07/01/2026).
 *
...
object Nanp {
    ...
    val AREA_CODES: Map<String, NanpEntry> by lazy {
        TABLE.splitToSequence(';').associate { line ->
            val parts = line.split(',')
            parts[0] to NanpEntry(
                country = parts[1],
                timezone = parts.getOrNull(2)?.takeIf { it.isNotEmpty() },
            )
        }
    }
    ...
    fun lookupAreaCode(e164: String): NanpEntry? {
        val match = E164_US_CA.find(e164) ?: return null
        return AREA_CODES[match.groupValues[1]]
    }
    ...
    /** "US" | "CA" | null — drives the registration-pending banner. */
    fun destinationCountry(e164: String): String? = lookupAreaCode(e164)?.country

    /**
     * Local wall-clock time at the destination's primary timezone, or null for
     * non-geographic/unknown codes — "unknown local time" shows no hint.
     */
    ...
    fun nationalDigits(input: String): String {
        ...
        val national = if (digits.startsWith("1")) digits.drop(1) else digits
        return national.take(10)
    }

    /** '(415) 555-0134' progressive as-you-type formatting of national digits. */
    fun formatAsYouType(digits: String): String = when {
        digits.isEmpty() -> ""
    ...
    /** `+1XXXXXXXXXX` for a complete 10-digit national number, else null. */
    fun toE164(input: String): String? {
        val digits = nationalDigits(input)
        return if (digits.length == 10) "+1$digits" else null
    }
}
