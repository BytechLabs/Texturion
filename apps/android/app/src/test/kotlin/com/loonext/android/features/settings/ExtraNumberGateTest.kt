...
class ExtraNumberGateTest {
    ...
    fun `a Canadian workspace may buy one, with no registration to wait on`() {
        ...
        assertNull(
            extraNumberBlockedReason(country = "CA", usTextingEnabled = false),
        )
    }

    @Test
    fun `a US workspace waits for carrier approval`() {
        val reason = extraNumberBlockedReason(country = "US", usTextingEnabled = false)
        assertTrue(
    ...
    fun `an approved US workspace may buy one`() {
        assertNull(
            extraNumberBlockedReason(country = "US", usTextingEnabled = true),
        )
    }
    ...
    fun `a country the provisioner cannot order in is refused`() {
        ...
        val reason = extraNumberBlockedReason(country = "GB", usTextingEnabled = true)
        assertTrue(
    ...
    fun `every refusal explains itself`() {
        ...
        for (args in listOf("US" to false, "GB" to true)) {
            val reason = extraNumberBlockedReason(args.first, args.second)
