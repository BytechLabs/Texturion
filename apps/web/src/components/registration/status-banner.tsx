...
/**
 * RegistrationStatusBanner (cross-track contract; DESIGN.md G7, SPEC §4.4):
 * the slim amber banner mounted above the inbox list. Self-fetching (the
 * registration + company hooks — the realtime provider invalidates both on
 * `registration.updated` / `number.updated`), links to /settings/numbers,
 * renders null when approved or not applicable, and fires the green
 * "You're live" toast on the observed approval transition (G7). Copy is the
 * SPEC §4.4 exact-string table via registration-ui-state.ts.
 */
export function RegistrationStatusBanner() {
  ...
  return (
    <div
      role="status"
      className="flex items-center gap-2.5 border-b border-warning/30 bg-warning/10 px-4 py-2"
    >
      <TriangleAlert
