...
export function ConsentVisual({ className }: { className?: string }) {
  return (
    ...
    >
      ...
      <div className="mt-3 rounded-lg border border-[color:var(--hairline)] bg-white px-3 py-2">
        <p className="text-[11px] text-[color:var(--ink-55)]">To</p>
        <p className="text-[15px] tabular-nums text-[color:var(--day-ink)]">
          (416) 555-0187
        </p>
      </div>

      {/* Consent attestation, required for a new contact (SPEC §5). */}
      <label className="mt-3 flex cursor-default items-start gap-2.5 rounded-lg bg-[color:var(--petrol-12)] p-3">
        <span
          className="mt-0.5 flex size-4 shrink-0 items-center justify-center rounded border border-[color:var(--petrol)] bg-[color:var(--petrol)] text-white"
          aria-hidden
        >
          <Check className="size-3" strokeWidth={3} />
        </span>
        <span className="text-[13px] leading-snug text-[color:var(--day-ink)]">
          This customer asked us to text them
          <span className="mt-0.5 block text-[12px] text-[color:var(--ink-55)]">
      ...
      <div className="mt-3 flex items-start gap-2.5 rounded-lg border border-[color:var(--hairline)] bg-white p-3">
        <span
          className="mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-full bg-[color:var(--petrol-12)] text-[color:var(--petrol)]"
          aria-hidden
        >
          <ShieldCheck className="size-3.5" strokeWidth={1.75} />
        </span>
        <p className="text-[13px] leading-snug text-[color:var(--day-ink)]">
          Consent on file
          <span className="mt-0.5 block text-[12px] text-[color:var(--ink-55)]">
