...
export function SavedRepliesVisual({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        "rounded-[10px] border border-[color:var(--hairline)] bg-white p-4 shadow-[0_24px_64px_-32px_rgba(28,25,23,0.25)]",
        className,
      )}
    >
      {/* Template popover, anchored above the composer, like the app's picker. */}
      <div className="overflow-hidden rounded-lg border border-[color:var(--hairline)] bg-white shadow-lg">
        <div className="flex items-center gap-2 border-b border-[color:var(--hairline)] px-3 py-2 text-[12px] text-[color:var(--ink-55)]">
        ...
        <ul className="max-h-[220px] py-1">
          {TEMPLATES.map((t, i) => (
            ...
            >
              <span className="text-[14px] font-medium text-[color:var(--day-ink)]">
                {t.name}
              </span>
              <span className="truncate text-[12px] text-[color:var(--ink-55)]">
                {t.preview}
              </span>
            </li>
          ))}
      ...
      <div className="mt-2 flex items-center gap-2 rounded-lg border border-[color:var(--hairline)] bg-white px-3 py-2">
        <span className="flex-1 text-[15px] text-[color:var(--day-ink)]">
          /on
          <span className="ml-0.5 inline-block h-4 w-px animate-pulse bg-[color:var(--petrol)] align-middle motion-reduce:animate-none" />
        </span>
        <span className="inline-flex items-center gap-1 rounded-md bg-[color:var(--petrol)] px-2 py-1 text-[12px] font-medium text-white">
          Send
          <CornerDownLeft className="size-3" strokeWidth={2} aria-hidden />
        </span>
      </div>
      <p className="mt-2 px-1 text-[12px] text-[color:var(--ink-55)]">
        Type <span className="font-medium text-[color:var(--day-ink)]">/</span> to open your
