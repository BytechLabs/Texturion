...
describe("seatLimit", () => {
  it("mirrors SPEC §2: Starter 3, Pro 15, NULL plan reads as Starter", () => {
    expect(seatLimit("starter")).toBe(3);
    expect(seatLimit("pro")).toBe(15);
...
describe("seatUsage", () => {
  ...
  it("never suggests an upgrade at Pro capacity (Enterprise is contact-sales, #83)", () => {
    // Pro caps at 15; it is the top self-serve plan, so the line shows the
    // ceiling but never the "Upgrade for more" nudge.
    const usage = seatUsage(14, 1, "pro");
    expect(usage.limit).toBe(15);
    expect(usage.full).toBe(true);
    expect(usage.line).toBe("15 of 15 seats");
    // Below the Pro cap it reads factually too.
    expect(seatUsage(8, 0, "pro").line).toBe("8 of 15 seats");
  });
