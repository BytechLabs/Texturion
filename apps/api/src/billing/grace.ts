...
/**
 * Insert the `(company_id, canceled_at, threshold_day)` ledger row FIRST and
 * only send when the insert actually landed — the §9 `subscription.deleted`
 * handler and the §11 cron share this ledger, so overlap can never
 * double-send. Returns whether this call sent the email.
 */
 ...
 * send the final email. State-gated for idempotency: when no non-released
 * number and no active campaign remain, the company was already processed and
 * nothing runs (§11: "status transitions are one-way; release skips
...
async function releaseExpiredCompany(
  ...
  if (numbersToRelease === 0 && !campaignActive) return; // already done
 ...
 * email. Work is selected by state (status + `canceled_at` age), never by
 * "last run" bookkeeping, so re-runs and overlaps are safe.
 *
