...
| **Supabase redirect URLs (native Google sign-in, #166)** | Supabase Dashboard → Authentication → URL Configuration → Redirect URLs: add all four — `com.loonext.android://auth-callback`, `com.loonext.android://auth-callback?state=*`, `com.loonext.ios://auth-callback`, `com.loonext.ios://auth-callback?state=*` (the wildcard entries are REQUIRED: the apps append a CSRF `state` nonce and the allow-list is exact-match without them). Until added, GoTrue silently falls back to the site URL — the browser strands on app.loonext.com and the app never sees a redirect. The Google provider itself already serves the web — no provider-side change. |
