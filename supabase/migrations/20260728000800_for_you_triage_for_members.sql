-- #416: we paged every member about an unassigned lead and then hid the
-- unassigned queue from them.
--
-- `notifications/inbound.ts` sends a new lead to "all active members", because
-- a new customer has no assignee. The triage strip showed that same work to
-- owners and admins only. So a member was woken for a lead and then could not
-- see it on the one screen whose job is answering "what needs me right now".
--
-- Each half was defensible, which is why it survived: notify-everyone serves
-- "never miss a lead", owner-only triage serves "the boss hands out work".
-- Nobody checked the seam.
--
-- DECISION (D53): SELF-SERVE. The crew answers unclaimed work.
--   * The product ALREADY treats them as the responders — the notification
--     goes to all of them.
--   * D12's ICP is 1-10 field staff. A crew of three has no dispatcher.
--   * #388's five-minute window does not survive waiting for an owner to wake
--     up, and speed-to-first-response is the marketed differentiator.
-- The queue follows the notification, rather than narrowing the notification
-- to follow the queue.
--
-- THE PREMISE THAT DIED WITH THE GATE, and why this needed care rather than
-- deleting a condition: triage_tasks carried no #106 number filter, on the
-- stated grounds that "leads are always unrestricted". That was true only
-- BECAUSE triage was owner/admin-only. Opening it to members makes the filter
-- load-bearing.
--
-- Rebuilt from 20260728000400. A NEW migration (D7/D14). The signature is
-- UNCHANGED and p_is_lead is now accepted and ignored, deliberately: the
-- migration and the Worker deploy separately, and a narrowed signature would
-- 404 every /for-you call made by the not-yet-deployed Worker in between.
-- Dropping the parameter is the contract half of that expand, filed as its own
-- issue rather than done here.
