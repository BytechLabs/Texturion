-- #293 — the focus queue stops counting what you deferred.
--
-- "Snoozed threads are excluded from ... the focus queue count, or the metric
-- lies in the other direction."
--
-- The exclusion goes in the `conv` CTE rather than in each section, because
-- waiting_on_you, unread and triage all read from it — three copies of one
-- predicate is how two of them end up disagreeing after the third is edited.
--
-- WHY TRIAGE TOO, even though triage is UNASSIGNED work. The snooze is mine and
-- the focus queue is mine: if I have looked at an unclaimed thread and decided
-- it is Thursday's problem, it should stop being on my queue today. It is
-- untouched for every colleague, whose own queue joins on their own user id.
--
-- One implementation, as before: the 6-arg shim (#454) forwards to this, so
-- both signatures answer identically for as long as both exist.
