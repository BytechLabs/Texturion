-- Searching a customer by phone number, written the way people write it.
--
-- Contacts matched a query only as a bare substring of the stored E.164 value,
-- so "6478923862" and "3862" found the contact but "647-892-3862",
-- "(647) 892-3862" and "892-3862" found nothing. The formatted spellings are
-- the ones that matter: they are how a number is read aloud, how it arrives in
-- a text, and how this product's own screens display it, so copying a number
-- out of the app and pasting it into search returned no results.
--
-- The fix compares digits to digits. Three digits is the floor, both because a
-- shorter fragment matches most of a contact list and because a GIN trigram
-- index cannot serve a LIKE pattern below trigram length.

-- Serves the digits-only LIKE below. regexp_replace is immutable, so the
-- expression is indexable.
create index if not exists contacts_phone_digits_trgm
  on public.contacts
  using gin ((regexp_replace(phone_e164, '[^0-9]', '', 'g')) extensions.gin_trgm_ops);
