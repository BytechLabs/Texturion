-- For You and the Inbox derived the SAME `unread` fact from two different
-- predicates, and they disagreed permanently.
--
-- 20260702000000_unread_excludes_own_sends.sql fixed the Inbox so that "your own
-- sends and notes never mark the thread unread for you". api_for_you was created
-- AFTER that fix and carried the old predicate anyway -- `last_message_at >
-- watermark`, which is exactly the expression that migration set out to kill --
-- and two later recreations copied it forward.
--
-- The visible bug: post a note from the task drawer without opening the thread,
-- and For You flags the conversation unread (petrol dot, "Unread", and it jumps
-- up the waiting-on-you queue) while the Inbox correctly does not. No refetch
-- heals it, because both surfaces are faithfully reporting different SQL.
--
-- This recreates api_for_you with the Inbox's predicate copied verbatim, so the
-- two cannot drift again. Signature and every other clause are unchanged.

CREATE OR REPLACE FUNCTION public.api_for_you(p_company_id uuid, p_user_id uuid, p_is_lead boolean, p_now timestamp with time zone, p_limit integer DEFAULT 20, p_hidden_number_ids uuid[] DEFAULT NULL::uuid[])
 RETURNS jsonb
 LANGUAGE sql
