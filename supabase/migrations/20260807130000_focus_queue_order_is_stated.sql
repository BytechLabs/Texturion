-- ===========================================================================
-- [#534] The focus queue says what order it is in.
--
-- Every section of this screen is built with jsonb_agg, and not one of those
-- aggregates said how to order its rows. Each relied on the ORDER BY inside its
-- own CTE surviving into the outer join that attaches contact names — and SQL
-- makes no such promise. A CTE's order is an implementation detail of producing
-- its rows, not a property the rows carry.
--
-- So the order of the post-login queue was whatever the query plan happened to
-- produce. "What needs you first" is the entire content of this screen: the
-- overdue-pinned thread has to come before the merely unread one, the oldest due
-- reminder before the newest, the most recent message before the older. Any of
-- those could arrive backwards.
--
-- FOUND BY A TEST THAT WAS ALREADY FAILING, on a database with realistic volume
-- in it. supabase/tests/for_you_notifications.test.sql asserts the unread
-- cross-cut is newest-first and got it oldest-first. The same test passes in CI,
-- which resets from empty — different statistics, different plan, favourable
-- order, green. It was wrongly written off once as local drift; the local
-- database was the only place telling the truth.
--
-- The fix is the one construct that guarantees an aggregate's input order:
-- ORDER BY inside jsonb_agg, on the same keys the CTE already declares. Nine
-- clauses across six sections. Nothing else changes.
--
