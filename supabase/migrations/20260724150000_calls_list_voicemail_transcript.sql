-- The calls list projection gains voicemail_transcript, so the log and the
-- thread can show what a voicemail SAYS without a second read per row. The
-- column is written best-effort after the recording is stored, so it is
-- nullable and its absence is never a reason to hide the audio.
--
-- Additive and nullable; the signature is unchanged, so CREATE OR REPLACE
-- suffices and every existing call site is untouched. Body otherwise identical
-- to 20260723002000 (answered_at).
