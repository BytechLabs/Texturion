...
- **Cloudflare → Caching → Cache Rules → create a rule** matching `http.host eq "loonext.com"`, **Eligible for cache**, Edge + Browser TTL = *use cache-control header if present*. This edge-caches the static marketing HTML (the app sends `Cache-Control: s-maxage=3600` on apex responses, `apps/web/next.config.ts`) so repeat requests skip the OpenNext cold-isolate (~1.3–1.9 s TTFB). The Deploy workflow purges the cache after each release (the `backend` job in `ship.yml`), which is why `CLOUDFLARE_API_TOKEN` needs **Cache Purge** and `CLOUDFLARE_ZONE_ID` is a secret. The Cloudflare rule is host-scoped to the apex. The Next `has` host predicate that pairs with it is anchored (`^loonext\.com$`) as of #559: OpenNext compiles that predicate to an unanchored regex, so a bare `loonext.com` also matched `app.loonext.com` and stamped the same `s-maxage` there — verified live. Nothing leaked (no `cookies()` or `next/headers` under `app/(app)`, so signed-in HTML is an empty client-fetched shell), but a stale app shell can point at chunk hashes a deploy has purged. Revocable public links (`/photos/…`) are excluded from the apex rule by path AND sent `no-store, private`, so revoking one takes effect at once rather than after the edge TTL.
